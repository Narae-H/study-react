...
<summary>shallowEqual (얕은 비교)</summary>

**shallowEqual 이란?**   
shallowEqual(objA, objB)는 두 객체의 **key-value**를 **1 depth**까지 비교.   
값 비교는 === **연산자(참조비교)**로 수행.
<br/>

**예제1. 얕은 비교가 통과하는 경우**
```js
// key 개수, key 이름, key 값 모두 같고
// value가 원시값(primitive value)이며, 값이 같음
//  ===로 비교했을 때 같음
...
```js
// a라는 key는 같지만
// value가 [1]로 배열이므로, 값은 같지만 참조값이 다름
// === 연산으로 false
shallowEqual({ a: [1] }, { a: [1] }); // false
```
<br/>

**주의점: depth 2 이상은 비교하지 않음**
```js
// a라는 key는 같지만
// value가 {b:1}은 객체로, 값은 같지만 참조값이 다름
// === 연산으로 false
shallowEqual({ a: { b: 1 } }, { a: { b: 1 } }); // ❌ false
```
<br/>

**useEffect()의 의존성과의 관계**
```js
useEffect((  ) => {
  // 실행코드
...
| 타입                   | 참조 방식            | 재렌더링 판단        | 비고               |
| -------------------- | ---------------- | -------------- | ---------------- |
| string, number 등 원시값 | 값 자체 비교 (`===`)  | 값이 동일하면 실행 안 됨 | 안정적              |
| 배열, 객체 등 참조값         | 참조 주소 비교 (`===`) | 참조가 바뀌면 실행됨    | 값은 같아도 새 객체면 실행됨 |


**왜 shallowEqual을 쓰는가?**
- Redux 상태나 React props가 객체/배열일 경우, 내부 값은 같지만 참조가 달라서 매번 `useEffect`가 실행될 수 있음.
- `useSelector(state => someObject, shallowEqual)`처럼 사용하면 불필요한 렌더링 방지에 유용.
